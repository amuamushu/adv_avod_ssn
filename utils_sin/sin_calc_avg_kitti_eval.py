...
# @Last Modified time: 2019-04-29 12:25:40
...
def calc_avg_scores(output_dir, model_config, eval_config, 
    ...
    if do_eval_sin:
        ...
        for sin_input_name in SINFields.SIN_INPUT_NAMES:
            ...
            # Calculate mean AP over repeats and write as a txt file
            fname_avg = '{}_{}_{}_{}_avg.txt'.format(model_config.checkpoint_name,
                                                     res_name,
                                                     str(score_threshold),
                                                     data_split)
            with open(os.path.join(res_dir,fname_avg),'w') as f_avg:
                for idx_ckpt,ckpt in enumerate(ckpt_list):
                    ...
                    # Write in the file
                    f_avg.write('<'+sin_input_name+' - checkpoint: '+ str(ckpt) + '>\n')
        ...
        # Find min(AP_lidar,AP_rgb) and save corresponding SEM
        print('   ... Writing final file')
        fname_final = os.path.join(output_dir,model_config.checkpoint_name,
                                   'offline_eval',res_name+'_sin_{}_{}_{}/{}_{}_{}_{}_min.txt'.format(
                                        eval_config.sin_type,
                                        eval_config.sin_level, 
                                        eval_config.sin_repeat,
                                        ...
                                        str(score_threshold),
                                        data_split))
        with open(fname_final,'w') as f_final:
            for idx_ckpt,ckpt in enumerate(ckpt_list):
                ...
                f_final.write('checkpoint: '+ str(ckpt) + '\n')
                for idx_eval_name, eval_name in enumerate(list_eval_names):
                    f_final.write(eval_name + ":" + \
                                  ','.join(['{:.6f} ({:.6f})'.format(val,se) for (val,se) \
                                            in zip(res_temp[idx_eval_name],sem_temp[idx_eval_name])]) +\
                                  '\n')
    else:
        # Eval AIN(All input noise mode)
        res_dir = os.path.join(output_dir,model_config.checkpoint_name,
                               'offline_eval',res_name+'_ain_{}_{}_{}/'.format(
                                    eval_config.sin_type,
                                    eval_config.sin_level, 
                                    eval_config.sin_repeat))
        fname = '{}_{}_{}_{}_{}_rep.txt'.format(model_config.checkpoint_name,
        ...
        res_dir = os.path.join(output_dir,model_config.checkpoint_name,
                               'offline_eval',res_name+'_ain_{}_{}_{}/'.format(
                                    ...
                                    eval_config.sin_repeat))
        # Initialize result dictionary
        dict_res = np.zeros((eval_config.sin_repeat,
                             len(ckpt_list),
                             len(list_eval_names),
                                 3))
        # Read result files for differen idx_repeat
        ...
        with open(os.path.join(res_dir,fname_avg),'w') as f_avg:
            for idx_ckpt,ckpt in enumerate(ckpt_list):
                # Calcuate mean AP over repeats
                res_temp = dict_res[:,idx_ckpt,:,:].mean(axis=0)
                dict_res_avg[ckpt] = res_temp
                # Calculate confidence intervl
                sem_temp = CI_FACTOR * sem(dict_res[:,idx_ckpt,:,:],axis=0)
                dict_res_sem[ckpt] = sem_temp
                # Write in the file
                f_avg.write('<AIN checkpoint: '+ str(ckpt) + '>\n')
                for idx_eval_name, eval_name in enumerate(list_eval_names):
                    f_avg.write(
                        eval_name + ":" + \
                        ','.join(['{:.6f} ({:.6f})'.format(val,se) for (val,se) \
...
def parse_args():
    default_output_dir = '../avod_data/kitti_avod/object/outputs'

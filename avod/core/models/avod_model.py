...
class AvodModel(model.DetectionModel):
    ...
    def build(self):
        ...
        with tf.compat.v1.variable_scope('avod_projection'):
            ...
            with tf.compat.v1.variable_scope('img'):
                image_shape = tf.cast(tf.shape(
                    input=rpn_model.placeholders[RpnModel.PL_IMG_INPUT])[0:2],
                    tf.float32)
        ...
        if self._config.is_adversarial: 
            ...
            delta = self.model_config.adv_epsilon * tf.sign(tf.gradients(cls_loss, img_rois))   
        ...
        with tf.compat.v1.variable_scope('avod_apply_mb_mask'):
            ...
            mb_classifications_softmax = tf.boolean_mask(
                tensor=all_cls_softmax, mask=mb_mask)

            # Offsets
            mb_offsets = tf.boolean_mask(tensor=all_offsets, mask=mb_mask)

            # Angle Vectors
            if all_angle_vectors is not None:
                mb_angle_vectors = tf.boolean_mask(tensor=all_angle_vectors, mask=mb_mask)
            else:
                mb_angle_vectors = None

        # Encode anchor offsets
        with tf.compat.v1.variable_scope('avod_encode_mb_anchors'):
            mb_anchors = tf.boolean_mask(tensor=top_anchors, mask=mb_mask)
            ...
            if self._box_rep == 'box_3d':
                ...
                mb_boxes_8c = tf.boolean_mask(tensor=proposal_boxes_8c, mask=mb_mask)
                ...
                # Get mini batch
                mb_boxes_4c = tf.boolean_mask(tensor=proposal_boxes_4c, mask=mb_mask)
        ...
        with tf.compat.v1.variable_scope('bev_avod_rois'):
            mb_bev_anchors_norm = tf.boolean_mask(
                tensor=bev_proposal_boxes_norm_tf_order, mask=mb_mask)
            ...
            tf.compat.v1.summary.image('bev_avod_rois',
        ...
        with tf.compat.v1.variable_scope('img_avod_rois'):
            ...
            mb_img_anchors_norm = tf.boolean_mask(
                tensor=img_proposal_boxes_norm_tf_order, mask=mb_mask)
            ...
            tf.compat.v1.summary.image('img_avod_rois',
                             img_input_rois,
                             max_outputs=avod_mini_batch_size)
        ...
        with tf.compat.v1.variable_scope('avod_nms'):
            ...
            all_top_scores = tf.reduce_max(input_tensor=all_cls_logits[:, 1:], axis=1)
    ...
    def sample_mini_batch(self, anchor_box_list_gt, anchor_box_list,
        ...
        with tf.compat.v1.variable_scope('avod_create_mb_mask'):
            ...
            max_ious = tf.reduce_max(input_tensor=all_ious, axis=0)
            max_iou_indices = tf.argmax(input=all_ious, axis=0)
            ...
            mb_gt_indices = tf.boolean_mask(tensor=max_iou_indices, mask=mb_mask)
